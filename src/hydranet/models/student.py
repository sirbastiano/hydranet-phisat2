...
def create_phisatnet(config: str = DEFAULT_STUDENT_CONFIG, **overrides: Any) -> PhisatNet:
    """
    ...
    """
    if config not in STUDENT_CONFIGS:
        raise ValueError(f"Unknown config: {config}. Available: {list(STUDENT_CONFIGS.keys())}")

    model_kwargs = STUDENT_CONFIGS[config].copy()
    if overrides:
        model_kwargs.update(overrides)

    return PhisatNet(**model_kwargs)


def list_phisatnet_configs() -> List[str]:
    """Return available student preset names."""
    return sorted(STUDENT_CONFIGS.keys())
